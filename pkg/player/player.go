...
func GetPlayerByID(id, tableName string, client dynamodbiface.DynamoDBAPI) (*Player, error) {
	...
	// if match not found return an error
	if !r.MatchString(id) {
	...
	// if a error is received return it
	if err != nil {
...
func CreatePlayer(req events.APIGatewayProxyRequest, tableName string, client dynamodbiface.DynamoDBAPI) (
	*Player,
	...
	// check if the player with the given id already exists or not
	...
	// if error in marshalling return the error
	if err != nil {
	...
	// if error is received, return it
	if err != nil {
